      ...
      <h2 className='mb-3'>Featured Products</h2>
      {
         ...
         <div className="row">
            {data?.data.data.map(product => {
               return <div key={product.id} className="col-lg-2 col-md-4 mb-4">
                  ...
                  <Link to={`/productDetail/${product.id}`} >
                     ...
                     <button onClick={()=>postToCart(product.id)} className="btn bg-main text-white w-100 btn-sm">Add to cart</button>
                  </div>
               </div>
            })}
         </div>
